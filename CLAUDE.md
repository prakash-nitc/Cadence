...
A single-user planning and discipline app. One Windows laptop, one Android phone.
Local-only. No accounts, no server, no sync.
...
6. **Design tokens only.** Colours come from the Tailwind theme names in SPEC §8
   (`ink`, `panel`, `edge`, `text`, `muted`, `signal`, `pass`, `warn`, `fail`). No raw hex
   in components, no arbitrary values like `bg-[#1a1a1a]`.
...
8. **Build in the order in SPEC §9** and stop at each session's done-condition. Do not
...
points, badges, cloud sync, or LLM coach.
